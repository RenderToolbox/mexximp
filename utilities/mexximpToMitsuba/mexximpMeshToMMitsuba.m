function [plyFile, plyRelativePath] = mexximpMeshToMMitsuba(scene, mesh, varargin)
%% Convert a mexximp mesh to a PLY file that we can pass to Mitsuba.
%
% plyFile = mexximpMeshToMMitsuba(scene, mesh) converts the given mexximp
...
%
% mexximpMeshToMMitsuba( ... 'workingFolder', workingFolder) specify the
% folder where PLY file will be written.  The default is pwd().
%
% mexximpMeshToMMitsuba( ... 'meshSubfolder', meshSubfolder) specify the
...
% workingFolder.
%
% mexximpMeshToMMitsuba( ... 'rewriteMeshData', rewriteMeshData)
